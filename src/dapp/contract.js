...
export default class Contract {
  ...
  initialize(callback) {
    this.web3.eth.getAccounts((error, accts) => {
      ...
      this.flightSuretyData.methods
        .callerAuthorized(this.config.appAddress)
        .call({ from: this.config.appAddress }, (err, res) => {
          console.log('callerAuthorized', res)
        })

      this.flightSuretyData.methods
        .airlineRegistered(this.airlines[0])
  ...
  buyInsurance(flightName, departure, ether, callback) {
    ...
    self.flightSuretyData.methods
      .buyInsurance(self.passengers[0], self.airlines[0], flightName, departure)
      .send(
        {
  ...
  getInsurance(flightName, departure, callback) {
    ...
    self.flightSuretyApp.methods
      .getInsurance(self.passengers[0], self.airlines[0], flightName, departure)
      .call(
        {
          from: self.passengers[0],
        },
