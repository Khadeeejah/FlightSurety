...
;(async () => {
  ...
  accounts.slice(16, 37).forEach(async (oracleAddress) => {
    ...
    try {
      ...
      let indexesResult = await flightSuretyApp.methods
        .getMyIndexes()
...
setTimeout(() => {
  oracles.forEach((oracle) => {
    console.log(
      `Oracle: address[${oracle.address}], indexes[${oracle.indexes}]`,
    )
  })
...
flightSuretyApp.events.OracleRequest(
  ...
  function (error, event) {
    if (error) {
      ...
      console.log(
        `OracleRequest: [${result.index}] for ${result.flight} ${result.timestamp}`,
      )
